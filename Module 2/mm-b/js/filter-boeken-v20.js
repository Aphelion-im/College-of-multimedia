...
window.addEventListener("load", function () {
  ...
  var cards = document.querySelectorAll('.boekenMain .card');
  // Resultaat output
  ...
  resultaat.innerText = 'Resultaten: ' + boeken.length;
    ...
    boekenFilter.addEventListener('click', function (e) {
      ...
      for (var card of cards) {
        ...
        if (sliderValue >= cardWaarde) {
          card.setAttribute('data-bedrag-flag', '1');
        } else {
        ...
        if (card.getAttribute('data-bedrag-flag') == 0 && sliderValue != 50) {
          bedrag_show = false;
        }
